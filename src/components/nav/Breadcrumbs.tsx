...
export default function Breadcrumbs({ dynamicLabel }: { dynamicLabel?: string }) {
  ...
  const isDetailRoute =
    !dynamicLabel &&
    (/^\/analyses\/.+/.test(pathname) || /^\/chats\/.+/.test(pathname) || /^\/specs\/.+/.test(pathname) || /^\/sequence-diagrams\/.+/.test(pathname) || /^\/test-plans\/.+/.test(pathname) || /^\/reviews\/.+/.test(pathname) || /^\/runbooks\/.+/.test(pathname));
